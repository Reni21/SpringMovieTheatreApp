// // Pin movie card to admin-schedule page without call server 2
function pinMovie(form, event, moviesToPin, submitFormHandler) {
    ...
    $.each(movieIds, function (index, value) {
        ...
        var link = '<div id="' + value + '"><div class="wrapper">' +
            ...
            '<p class="movie-duration">' + msgDictionary.get('duration') + ': ' + movie.duration + ' min</p>' +
            '<form action="movie-session?movieId=' + value + '&date=' + date + '"' +
...
// Unpin movie card from admin-schedule page with ajax
